function pokemonApi(urlAPI = 'https://pokeapi.co/api/v2/pokemon/') {
    document.querySelector("#datosweb").innerHTML = ""
...
function siguienteAnterior(data_next, data_previous) {
    ...
    document.querySelector("#botonAS").innerHTML = `
                            <button class="btn btn-primary mb-5   fs-1" ${disabledBotonPrev} onclick="pokemonApi('${data_previous}')">Anterior</button>
                            <button class="btn btn-primary mb-5   fs-1" ${disabledBotonNext} onclick="pokemonApi('${data_next}')">Siguiente</button>`
}
